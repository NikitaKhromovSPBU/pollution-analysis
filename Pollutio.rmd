...
```{r}
df.new <- df.new |> mutate(CLIM = PREC > median(PREC))
```
## NP-plots для малого количества осадков
```{r}
p1 <- df.new |> filter(CLIM == FALSE) |> ggplot(aes(sample = MORT)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("MORT")
p2 <- df.new |> filter(CLIM == FALSE) |> ggplot(aes(sample = NOX)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("NOX")
p3 <- df.new |> filter(CLIM == FALSE) |> ggplot(aes(sample = POOR)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("POOR")
p4 <- df.new |> filter(CLIM == FALSE) |> ggplot(aes(sample = NONW)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("NONW")
p5 <- df.new |> filter(CLIM == FALSE) |> ggplot(aes(sample = DENS)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("DENS")
p6 <- df.new |> filter(CLIM == FALSE) |> ggplot(aes(sample = HOUS)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("HOUS")
p7 <- df.new |> filter(CLIM == FALSE) |> ggplot(aes(sample = EDUC)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("EDUC")
p8 <- df.new |> filter(CLIM == FALSE) |> ggplot(aes(sample = POPN)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("POPN")
p9 <- df.new |> filter(CLIM == FALSE) |> ggplot(aes(sample = OVR65)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("OVR65")
p10 <- df.new |> filter(CLIM == FALSE) |> ggplot(aes(sample = PREC)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("PREC")
...
```{r}
p1 <- df.new |> filter(CLIM == TRUE) |> ggplot(aes(sample = MORT)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("MORT")
p2 <- df.new |> filter(CLIM == TRUE) |> ggplot(aes(sample = NOX)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("NOX")
p3 <- df.new |> filter(CLIM == TRUE) |> ggplot(aes(sample = POOR)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("POOR")
p4 <- df.new |> filter(CLIM == TRUE) |> ggplot(aes(sample = NONW)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("NONW")
p5 <- df.new |> filter(CLIM == TRUE) |> ggplot(aes(sample = DENS)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("DENS")
p6 <- df.new |> filter(CLIM == TRUE) |> ggplot(aes(sample = HOUS)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("HOUS")
p7 <- df.new |> filter(CLIM == TRUE) |> ggplot(aes(sample = EDUC)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("EDUC")
p8 <- df.new |> filter(CLIM == TRUE) |> ggplot(aes(sample = POPN)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("POPN")
p9 <- df.new |> filter(CLIM == TRUE) |> ggplot(aes(sample = OVR65)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("OVR65")
p10 <- df.new |> filter(CLIM == TRUE) |> ggplot(aes(sample = PREC)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle("PREC")
ggarrange(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, ncol = 5, nrow = 2)
...
```{r}
shap.p.df <- df.new |> group_by(CLIM) |> select(NOX, POPN, NONW, MORT, CLIM) |>
...
```{r}
lil.p.df <- df.new |> group_by(CLIM) |> select(NOX, POPN, NONW, MORT, CLIM) |>
...
```{r}
ad.p.df <- df.new |> group_by(CLIM) |> select(NOX, POPN, NONW, MORT, CLIM) |>
...
## Тесты на совпадение распределений в группах
### t-test
```{r}
t.p.df <- df.new |> select(NOX, POPN, NONW, MORT, CLIM) |>
        summarize(NOX = t.test(NOX[CLIM == FALSE], NOX[CLIM == TRUE])$p.value,
                  POPN = t.test(POPN[CLIM == FALSE], POPN[CLIM == TRUE])$p.value,
                  NONW = t.test(NONW[CLIM == FALSE], NONW[CLIM == TRUE])$p.value,
                  MORT = t.test(MORT[CLIM == FALSE], MORT[CLIM == TRUE])$p.value)
t.p.df
```
### Wilcox
```{r}
wc.p.df <- df.new |> select(NOX, POPN, NONW, MORT, CLIM) |>
        summarize(NOX = wilcox.test(NOX[CLIM == FALSE], NOX[CLIM == TRUE])$p.value,
                  POPN = wilcox.test(POPN[CLIM == FALSE], POPN[CLIM == TRUE])$p.value,
                  NONW = wilcox.test(NONW[CLIM == FALSE], NONW[CLIM == TRUE])$p.value,
                  MORT = wilcox.test(MORT[CLIM == FALSE], MORT[CLIM == TRUE])$p.value)
wc.p.df
```