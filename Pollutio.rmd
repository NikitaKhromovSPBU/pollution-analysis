...
#         mutate(CLIM = factor(ifelse(PREC > median(PREC), ">.5", "<.5"), levels = c("<.5", ">.5")))
...
df.scale <- df.new |>
  select(-CLIM) |>
  mutate(`CLIMTemper` = as.integer(df.new$CLIM == "<.5")) |>
  scale() |>
  as.data.frame()
linear.model.scale.all <- lm(MORT ~ ., data = df.scale)
linear.model.scale.all.beta <- lm.beta(linear.model.scale.all)
...
```{r}
cbind(ols_vif_tol(linear.model.scale.all), ols_correlations(linear.model.scale.all)) |>
  as.data.frame() |>
...
### Модель без ***OVR65***, ***POOR***, ***CLIMTemper***
```{r}
linear.model.scale <- lm(MORT ~ . - OVR65 - POOR - CLIMTemper, data = df.scale)
...
cov.beta |>
  cov2cor() |>
  corrplot(method = "number", tl.col = "black", tl.srt = 60)
confidenceEllipse(lm.fin, which.coef = c("NOX", "NONW"), levels = 0.95, col = "black", xlim = c(0, 0.8), ylim = c(0, 0.8))
lines(c(-1, 1), c(-1, 1))
```
...
lm.fin.dirty <- lm.beta(lm(MORT ~ PREC + EDUC + NONW + NOX, data = df.dirty))
plot(lm.fin.dirty)
```

## Выбросы
Ранее за выбросы считались следующие точки.
```{r}
sort(outliers)
```
Проверим правильно ли они были выбраны.
